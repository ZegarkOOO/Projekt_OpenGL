...
#include "testowy123/OBJloader.h"
...
void szescian(void)
{
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
	{
		...
		*/
		...
		GLfloat sb2[3] = { 10 * cos(3.14 / xx), 0.0f, 10 * sin(3.14 / xx) };
		GLfloat sc2[3] = { 10 * cos(3.14 / xx), 10.0f, 10 * sin(3.14 / xx) };
...
void most(void) {
	glPolygonMode(GL_FRONT, GL_FILL);
	{
		...
		//droga na moscie (niebieska sciana)
		...
		//druga œciana boczna (zielona)
		glColor3d(0.0, 1.0, 0.0);
		glBegin(GL_TRIANGLE_STRIP);
		for (int z = 0; z < 40; z += 10) {
			for (int y = 0; y <= 40; y += 10) {
				glVertex3d(80, y, z);
				glVertex3d(80, y, z + 10);
			}
		}
		glEnd();
		...
		//druga sciana z tunelem z boku
		glColor3d(0.0, 1.0, 1.0);
		glBegin(GL_TRIANGLE_STRIP);
		for (int z = 0; z < 40; z += 10) {
			for (int x = 60; x <= 80; x += 10) {
				glVertex3d(x, 40, z);
				glVertex3d(x, 40, z + 10);
			}
		}
		glEnd();

		//trzecia sciana z tunelem z boku
		...
		//czwarta sciana z tunelem z boku
		...
		glBegin(GL_TRIANGLE_FAN);
		glVertex3d(60, 0, 20);
		for (double alpha = 0; alpha <= 90*3.14/180; alpha += 0.05)
		{
			double z = 20 * sin(alpha);
			double x = 20 * cos(alpha);
			glVertex3d(40 + x, 0, z);
		}
		glEnd();
		...
		for (double alpha = 0; alpha <= 90 * 3.14 / 180; alpha += 0.05)
		{
			double z = 20 * sin(alpha);
			double x = 20 * cos(alpha);
			glVertex3d(40 + x, 40, z);
		}
		glEnd();
		
		//trzecia sciana lukowa
		glColor3d(0.5, 0.2, 1.0);
		glBegin(GL_TRIANGLE_FAN);
		glVertex3d(20, 0, 20);
		for (double alpha = 90 * 3.1415 / 180; alpha <= 180 * 3.14 / 180; alpha += 0.05)
		{
			double z = 20 * sin(alpha);
			double x = 20 * cos(alpha);
			glVertex3d(40 + x, 0, z);
		...
		//czwarta sciana lukowa
		glColor3d(1.0, 0.5, 1.0);
		glBegin(GL_TRIANGLE_FAN);
		glVertex3d(20, 40, 20);
		for (double alpha = 90 * 3.14 / 180; alpha <= 180 * 3.14 / 180; alpha += 0.05)
		{
			double z = 20 * sin(alpha);
			double x = 20 * cos(alpha);
			glVertex3d(40 + x, 40, z);
		...
		glBegin(GL_TRIANGLE_STRIP);
		for (double alpha = 0; alpha <= 180 * 3.14 / 180; alpha += 0.01)
		{
			double z = 20 * sin(alpha);
			double x = 20 * cos(alpha);
			glVertex3d(40 + x, 0, z);
...
//zmienne globalne rozmiarow cegly i odstepow
float weight = 16;
float height = 6;
float length = 24;
float space = 1;
...
void cegla(float x, float y, float z) {
	glColor3d(0.59, 0.15, 0.15); //kolor scian bocznych
	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
	{
		...
		glVertex3f(x + length, y + weight, z);
		glVertex3f(x + length, y + weight, z + height);
		glVertex3f(x, y + weight, z);
		glVertex3f(x, y + weight, z + height);
		glVertex3f(x, y, z);
		glVertex3f(x, y, z + height);
		glEnd();
...
void mur(int ile_warst, int ile_ceg) {
	for (int j = 0; j < ile_warst; j++)	{
		if (j % 2 == 0) {
			for (int i = 0; i <= ile_ceg; i++) {
				cegla((length + space) * i, 0, j * (height + space));
			}
		}
		else {
			for (int i = 0; i < ile_ceg; i++) {
				cegla((length + space) * i + (length/2), 0, j * (height + space));
			}
		}
	}
}
